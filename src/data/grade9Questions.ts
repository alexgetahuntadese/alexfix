...
const buildChapterQuestions = (subject: string, chapter: string): Grade9Question[] => {
  ...
  if (subject === "Chemistry") {
    return [
      make("Easy", 1, `Which Grade 9 subject includes "${title}"?`, subject, others, `"${title}" is part of Grade 9 ${subject}.`, "Example: Chemistry studies matter and its changes.", ""),
      make("Easy", 2, `What is the main focus of "${title}"?`, descriptor.concept, peers.map((item) => item.descriptor.concept), `This chapter mainly teaches ${descriptor.concept}.`, "Example: Understanding atomic structure and bonding.", ""),
      make("Easy", 3, `Which learning goal best matches "${title}"?`, descriptor.skill, peers.map((item) => item.descriptor.skill), `Students study this chapter to ${descriptor.skill}.`, "Example: Students should be able to identify elements.", ""),
      make("Easy", 4, `Which real-life application best connects with "${title}"?`, descriptor.application, peers.map((item) => item.descriptor.application), `The chapter connects best with ${descriptor.application}.`, "Example: Understanding chemical reactions in daily life.", ""),
      make("Easy", 5, `Which tool is most useful when studying "${title}"?`, descriptor.tool, peers.map((item) => item.descriptor.tool), `${descriptor.tool} is essential for this chapter.`, "Example: Using periodic table effectively.", ""),
      make("Easy", 6, `Which statement best summarizes "${title}"?`, `It develops understanding of ${descriptor.concept}.`, peers.slice(0, 3).map((item) => `It develops understanding of ${item.descriptor.concept}.`), `That option best captures the chapter theme.`, "Example: The chapter builds a foundation for chemical understanding.", ""),
      make("Easy", 7, `A student who studies "${title}" well should be able to:`, descriptor.skill, peers.map((item) => item.descriptor.skill), `The chapter helps learners ${descriptor.skill}.`, "Example: Classify elements by their properties.", ""),
      make("Easy", 8, `Which type of evidence appears most often in "${title}"?`, descriptor.evidence, peers.map((item) => item.descriptor.evidence), `${descriptor.evidence} supports understanding in this chapter.`, "Example: Experimental data and chemical equations.", ""),
      make("Easy", 9, `Which chapter from the same subject focuses more on ${peer(0).descriptor.concept}?`, peer(0).title, [peer(1).title, peer(2).title, title], `"${peer(0).title}" focuses more on ${peer(0).descriptor.concept}.`, "Example: Bonding chapter builds on atomic concepts.", ""),
      make("Easy", 10, `What is a common misconception in "${title}"?`, descriptor.misconception, peers.map((item) => item.descriptor.misconception), `That misconception can hinder understanding.`, "Example: Thinking atoms are solid balls.", ""),

      make("Medium", 1, `How does "${title}" relate to industry?`, `It explains chemical principles in ${descriptor.application}.`, peers.map((item) => `It explains chemical principles in ${item.descriptor.application}.`), `Chemistry underlies industrial processes.`, "Example: Manufacturing and production applications.", ""),
      make("Medium", 2, `What are the safety considerations in "${title}" experiments?`, `Follow proper lab procedures and use protective equipment.`, peers.map((item) => `Follow proper lab procedures and use protective equipment.`), `Chemical experiments require safety awareness.`, "Example: Wearing goggles and lab coats.", ""),
      make("Medium", 3, `How does "${title}" connect to environmental science?`, `It provides a foundation for understanding ${descriptor.application}.`, peers.map((item) => `It provides a foundation for understanding ${item.descriptor.application}.`), `Chemistry is fundamental to environmental science.`, "Example: Pollution and chemical reactions.", ""),
      make("Medium", 4, `What careers use knowledge from "${title}"?`, descriptor.application, peers.map((item) => item.descriptor.application), `${descriptor.application} careers require this knowledge.`, "Example: Pharmacy and chemical engineering.", ""),
      make("Medium", 5, `How can "${title}" knowledge improve daily life?`, descriptor.application, peers.map((item) => item.descriptor.application), `Apply chemistry to ${descriptor.application}.`, "Example: Understanding household chemicals.", ""),
      make("Medium", 6, `What role does technology play in "${title}"?`, `Modern ${descriptor.tool} enhance chemical research.`, peers.map((item) => `Modern ${item.descriptor.tool} enhance chemical research.`), `Technology advances chemical understanding.`, "Example: Spectroscopy and analysis tools.", ""),
      make("Medium", 7, `How does "${title}" explain material properties?`, `It describes ${descriptor.concept} in materials.`, peers.map((item) => `It describes ${item.descriptor.concept} in materials.`), `Chemistry explains material behavior.`, "Example: Why metals conduct electricity.", ""),
      make("Medium", 8, `What are the practical applications of "${title}"?`, descriptor.application, peers.map((item) => item.descriptor.application), `Apply chemical principles to ${descriptor.application}.`, "Example: Medicine and materials science.", ""),
      make("Medium", 9, `How does "${title}" connect to biology?`, `It studies chemical interactions in ${descriptor.application}.`, peers.map((item) => `It studies chemical interactions in ${item.descriptor.application}.`), `Chemistry and biology are interconnected.`, "Example: Biochemical processes in cells.", ""),
      make("Medium", 10, `What mathematical skills are needed for "${title}"?`, `Calculate ${descriptor.evidence} using formulas.`, peers.map((item) => `Calculate ${item.descriptor.evidence} using formulas.`), `Chemistry involves quantitative calculations.`, "Example: Molar mass calculations.", ""),

      make("Hard", 1, `Design an experiment investigating ${descriptor.concept} in ${descriptor.application}.`, `Use controlled methods and ${descriptor.tool}.`, peers.map((item) => `Use controlled methods and ${item.descriptor.tool}.`), `Chemical experiments require careful design.`, "Example: Reaction rate investigation.", ""),
      make("Hard", 2, `Analyze the impact of chemical processes on ${descriptor.concept}.`, `Consider environmental and health effects.`, peers.map((item) => `Consider environmental and health effects.`), `Chemical processes affect systems.`, "Example: Industrial pollution effects.", ""),
      make("Hard", 3, `Evaluate the effectiveness of ${descriptor.application} methods.`, `Assess reaction outcomes and yields.`, peers.map((item) => `Assess reaction outcomes and yields.`), `Chemical process effectiveness requires evaluation.`, "Example: Synthesis efficiency rates.", ""),
      make("Hard", 4, `How might temperature affect "${title}"?`, `Temperature changes impact ${descriptor.concept}.`, peers.map((item) => `Temperature changes impact ${item.descriptor.concept}.`), `Temperature affects chemical systems.`, "Example: Reaction rate changes.", ""),
      make("Hard", 5, `What are the environmental implications in "${title}"?`, `${descriptor.concept} processes raise sustainability questions.`, peers.map((item) => `${item.descriptor.concept} processes raise sustainability questions.`), `Chemical processes have environmental implications.`, "Example: Green chemistry principles.", ""),
      make("Hard", 6, `How does "${title}" contribute to material science?`, `It informs ${descriptor.application} development.`, peers.map((item) => `It informs ${item.descriptor.application} development.`), `Chemistry is crucial for material science.`, "Example: New material synthesis.", ""),
      make("Hard", 7, `What are the quantum aspects of "${title}"?`, `${descriptor.concept} behavior follows quantum principles.`, peers.map((item) => `${item.descriptor.concept} behavior follows quantum principles.`), `Quantum mechanics explains chemical behavior.`, "Example: Electron orbital behavior.", ""),
      make("Hard", 8, `How can "${title}" principles be applied to medicine?`, `Apply ${descriptor.concept} in ${descriptor.application}.`, peers.map((item) => `Apply ${item.descriptor.concept} in ${item.descriptor.application}.`), `Medicine uses chemical principles.`, "Example: Drug design and interactions.", ""),
      make("Hard", 9, `Evaluate the sustainability of ${descriptor.application} processes.`, `Assess environmental impact and resource efficiency.`, peers.map((item) => `Assess environmental impact and resource efficiency.`), `Sustainable chemistry balances ecology and industry.`, "Example: Green synthesis methods.", ""),
      make("Hard", 10, `Propose a solution to a chemical problem using "${title}" knowledge.`, `Apply chemical principles to solve ${descriptor.application}.`, peers.map((item) => `Apply chemical principles to solve ${item.descriptor.application}.`), `Chemical knowledge solves real problems.`, "Example: Pollution reduction strategy.", ""),
    ];
  }
